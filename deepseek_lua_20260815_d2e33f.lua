...
MenuFrame.Size = UDim2.new(0, 300, 0, 460)
...
CreateToggle(MenuFrame, y, "Target Bots", Settings.TargetBots, function(val)
...
local function IsEnemyModelFast(model)
    ...
    if IsSameTeamModel(model) then return false end

    -- player check: if belongs to teammate and team check on
    for _, p in pairs(Players:GetPlayers()) do
        if p.Character == model and p ~= LocalPlayer then
    ...
    -- bot: if TargetBots on, treat as enemy if team check doesn't filter
    if not Settings.TargetBots then return false end
    return true
end
...
local function RefreshTargets()
    ...
    local function addModel(model, name)
        ...
        if IsEnemyModelFast(model) then
...
-- [[ AIMBOT ]]
local function GetNearestTargetFromCache()
    local nearest = nil
...
RunService.RenderStepped:Connect(function()
    if Settings.Aimbot and UserInputService:IsMouseButtonPressed(Settings.AimbotKey) then
        local target = GetNearestTargetFromCache()
        if target then
